...
public class SimpleDatabase implements EntryContentConstants
{
    ...
    private boolean bUsingBuffer = false;
    ...
    public SimpleDatabase(BufferedInputStream pBufferedInputStream)
    {
    	this.bFileOpen = true;
    	this.bUsingBuffer = true;
    	this.buff = pBufferedInputStream;
    ...
    public boolean done() throws ParserException
    {
        ...
        try
        {
            if((database != null && (bFileOpen & database.canRead())) || (bFileOpen & this.bUsingBuffer))
            {
                buff.mark(5);
