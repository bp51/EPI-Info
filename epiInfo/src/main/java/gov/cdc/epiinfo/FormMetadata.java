	...
	public FormMetadata(String viewXmlFile, Activity container)
	{
		try
		{
			...
			{ 
				...
				if (Height == 780 && Width == 549)
				{
					IsInterviewForm = true;
				}
				else
				{
					IsInterviewForm = false;
					...
					for (int x=0;x<obj_nod_list.getLength();x++)
					{
						...
						if (test != null)
						{
							...
							try
							{
								...
								shouldRepeatLast = obj_nod_list.item(x).getAttributes().getNamedItem("ShouldRepeatLast").getNodeValue().equals("True") ? true : false;
							}
							catch (Exception ex)
							{
								// 
