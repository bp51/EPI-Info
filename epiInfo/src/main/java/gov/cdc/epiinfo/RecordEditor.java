...
public class RecordEditor extends ActionBarActivity implements ICheckCodeHost 
{
		...
		if (!field.getType().equalsIgnoreCase("2") && !field.getType().equalsIgnoreCase("21"))
		{
			...
			else if (field.getType().equalsIgnoreCase("17") || field.getType().equalsIgnoreCase("18") || field.getType().equalsIgnoreCase("19"))
			{
				...
				try
				{
					...
					{
						int pos = ((ArrayAdapter)spn.getAdapter()).getPosition(pValue);
	...
	public Object GetValue(String pName) 
	{
		...
		if(temp != null)
		{
			...
			{
				result = (((RadioGroup)temp).getCheckedRadioButtonId() % 100) + "";
			}
			else if(temp instanceof Spinner)
			{
				...
				if(result != null)
				{
					...
					if(field.getType().equals("11"))
					{
						...
						{
							result = result.toString().equalsIgnoreCase(this.getString(R.string.yes)) ? true : false;
						}
					}
				}
			}
