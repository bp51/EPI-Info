...
public class PieChartView extends RelativeLayout {
	...
	{	
		...
		final PieChart pie = (PieChart) findViewById(R.id.plot);
				...
				if (pos > 0)
				{
					...
					if (c.moveToFirst())
					{
						...
						do
						{
							...
							pie.addSeries(new Segment(val + " (" + count + ")",count), new SegmentFormatter(colors[counter % 12], colors[counter % 12] - 0x33000000, context));
							counter++;
						}while(c.moveToNext());
						try
						{
