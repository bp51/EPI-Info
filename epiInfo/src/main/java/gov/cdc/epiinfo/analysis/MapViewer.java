...
public class MapViewer extends RelativeLayout {
	...
	{		
		...
		MapView mapView = (MapView) findViewById(R.id.mapControl);
		...
		Spinner latitudeSpinner = (Spinner) findViewById(R.id.latitudeField);
		...
		Spinner longitudeSpinner = (Spinner) findViewById(R.id.longitudeField);
	...
	private void GenerateMap()
	{
		try
		{
			...
			if (latitudeSpinner.getSelectedItemPosition() > 0 && longitudeSpinner.getSelectedItemPosition() > 0)
			{
				...
				{
					double latitude = (Double)latArray[x];
					double longitude = (Double)longArray[x];
