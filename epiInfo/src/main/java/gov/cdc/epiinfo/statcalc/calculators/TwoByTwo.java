...
public class TwoByTwo {
    ...
    public double Comb(double y, double x)
    {
        ...
        f = 1.0;
        for (i = 1; i <= Math.round((double)Math.min(x, y - x)); i++)
        {
            f = f * y / (double)i;
            y = y - 1.0;
        }
    ...
    public void BinomialExpansion(double c0, double c1, int f, Ref<double[]> refP, int degp)
    {
        ...
        for (i = degp - 1; i >= 0; i--)
        {
            p[i] = p[i + 1] * c0 * (i + 1) / (double)(c1 * (degp - i));
        }
    ...
    public double Func(double r)
    {
        ...
        if (r <= 1)
        {
            return numer / (double)denom - (value);
        }
        else
        {
            return (numer / Math.pow(r, (degD - degN)) / (double)denom) - value;
        }
    }
    ...
    public TableResults AnalyzeSingleTable(double yy, double ny, double yn, double nn)
    {
        ...
        double csc = (t * Math.pow(Math.abs((yy * nn) - (ny * yn)) - t / 2.0, 2.0)) / (c1 * c2 * r1 * r2);
